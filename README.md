...
| 文件 | 所属模块 | 内容 | 写入方式 |
|------|---------|------|---------|
| `data/runtime_state.json` | WorldManager | `active_world` | 原子写入 |
| `data/provider_state.json` | LLMRouter | `mode`, `manual_provider`, 失败/冷却/耗尽 | 原子写入 |
| `data/sessions/{world}_chat.json` | MemoryManager | 短期对话上下文 | 原子写入+备份+空保护 |
| `data/memory/{world}_long_term.json` | MemoryManager | 长期记忆 | 原子写入+备份+垃圾过滤 |
| `providers.yaml` | LLMRouter | provider 配置 | 原子写入+校验+热重载 |
| `.env` | Config Center | 行为参数 | 原子写入+备份+读回校验 |

**线程安全**: 所有状态文件操作均带锁保护（`memory_lock`, `provider_lock`, `world_lock`），Web 操作和 Bot 聊天并发不会损坏数据。
...
* [x] Telegram Bot
* [x] 多世界系统 + 热切换 (WorldManager)
* [x] 短期记忆 + 长期记忆 (MemoryManager)
* [x] 自动记忆抽取与压缩
* [x] 多模型供应商路由 (LLM Router + providers.yaml)
* [x] NPC 主动行为系统
* [x] 关系网络系统
* [x] Web 管理面板
* [x] 配置中心（行为参数管理）
