...
用户 ──→ Telegram ──→ telegram_handlers.py ──→ LLMClient (兼容层) ──→ LLMRouter
 ...
 │                    runtime_state.json
 │
 ├── RelationshipManager (关系唯一源, RLock + revision + 回滚)
 │
...
RelationshipManager ：关系数据唯一权威源，RLock + 回滚保护，
                      Web/AI/注入 全部走同一实例；写后校验 JSON 一致性。
...
关系数值只作为内部导演数据注入 Prompt，角色正文不得直接报告总值或变化量。自动抽取只产生 `delta`；应用时明确记录 `before_value → delta → after_value`。每次有效 Web/AI 修改都会递增持久化 `revision`，异步抽取若发现启动版本与应用版本不一致会安全丢弃，避免旧结果覆盖或污染 Web 手动设置。
...
| 关系网络 | `/relations` | 6 维度关系编辑 + revision/JSON/Prompt 只读调试信息 |
